...
3. 若你已提供 `srt` 且文字更可信，可开启“使用提供的SRT修正自动识别文字（保留自动时间轴）”。
   - 这时程序会先自动识别整条视频，保留自动识别的时间轴和分句节奏，再用你提供的 `srt` 修正文案。
...
- `使用提供的SRT修正自动识别文字` 模式会保留自动识别时间轴；高置信片段直接用提供的 `srt` 覆盖，部分中置信但长度结构接近的片段也会做保守替换，用来纠正常见术语错字，同时不改动时间轴。
